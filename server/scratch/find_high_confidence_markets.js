...
async function run() {
  console.log('🚀 INITIALIZING SCANNER FOR ALL BINANCE FUTURES MARKETS...');
  
  // 1. Fetch general news once for sentiment analysis
  let newsArticles = [];
  try {
    ...
    if (response.data && Array.isArray(response.data.Data)) {
      ...
      console.log(`✅ Loaded ${newsArticles.length} latest news articles from CryptoCompare`);
    } else {
      console.warn(`⚠️ CryptoCompare returned unexpected format:`, response.data);
    }
  } catch (err) {
    console.warn(`⚠️ News fetch failed: ${err.message}. Sentiment will default to neutral.`);
  }

  // 2. Fetch tickers from Binance to filter and rank by volume
  ...
  const activeUSDTPerps = Object.values(markets)
  ...
  activeUSDTPerps.sort((a, b) => b.volume24h - a.volume24h);
  
  // Select supported assets + top 40 other highest volume assets to scan
  const supportedCleanNames = new Set(SUPPORTED_ASSETS);
  const targetMarkets = [];
  
  // Always include supported assets
  for (const asset of SUPPORTED_ASSETS) {
    const marketObj = activeUSDTPerps.find(m => m.cleanName === asset);
    if (marketObj) {
      targetMarkets.push(marketObj);
    }
  }
  
  // Add other top volume assets that aren't already included
  let addedCount = 0;
  for (const m of activeUSDTPerps) {
    if (!targetMarkets.some(tm => tm.symbol === m.symbol)) {
      targetMarkets.push(m);
      addedCount++;
    }
    if (addedCount >= 40) break; // Limit additional assets to top 40
  }
  
  console.log(`\n📋 SCAN TARGETS ESTABLISHED: Total of ${targetMarkets.length} markets`);
  console.log(`- Supported core/meme assets: ${SUPPORTED_ASSETS.length}`);
  console.log(`- Extra high-volume perps: ${targetMarkets.length - SUPPORTED_ASSETS.length}`);
  ...
  console.log('\n🔍 SCANNING CANDLES AND COMPUTING AGENT SIGNALS...');
  
  for (let i = 0; i < targetMarkets.length; i++) {
    const target = targetMarkets[i];
    const assetClean = target.cleanName;
    const progress = `[${i + 1}/${targetMarkets.length}]`;
    ...
    try {
      ...
      if (candles.length < 30) {
        // console.log(`${progress} ${target.symbol}: Insufficient candles (${candles.length})`);
        continue;
      }
      ...
      const isSupported = supportedCleanNames.has(assetClean);
      ...
      // If fused confidence >= 0.40
      if (fusedConfidence >= 0.40) {
        matches.push(resultObj);
        console.log(`✨ FOUND HIGH CONFIDENCE MARKET: ${target.symbol} | Action: ${fusedAction} | Confidence: ${(fusedConfidence * 100).toFixed(1)}%`);
      }
      ...
      console.warn(`⚠️ Error scanning ${target.symbol}: ${err.message}`);
    }
  }
  
  console.log('\n======================================================');
  console.log(`📊 SCAN COMPLETED! Evaluated ${allResults.length} markets.`);
  console.log(`🎯 Found ${matches.length} markets with 40% or higher fused confidence level.`);
  console.log('======================================================\n');
  
  // Sort matches by confidence level descending
  matches.sort((a, b) => b.fusion.confidence - a.fusion.confidence);
  
  if (matches.length > 0) {
    console.log('=== 🏆 HIGH CONFIDENCE FUTURES MARKETS (>= 40%) ===');
    for (const m of matches) {
      const typeStr = m.isSupported ? '[Supported]' : '[Extra Perp]';
      console.log(`${typeStr} ${m.symbol} | fusedAction: ${m.fusion.action} | fusedConfidence: ${(m.fusion.confidence * 100).toFixed(1)}% | Composite: ${m.fusion.composite.toFixed(3)}`);
      console.log(`   └─ Technical Agent : ${m.technical.action} (conf: ${(m.technical.confidence * 100).toFixed(0)}%)`);
      console.log(`   └─ Sentiment Agent : ${m.sentiment.label.toUpperCase()} (conf: ${(m.sentiment.confidence * 100).toFixed(0)}%, articles: ${m.sentiment.articleCount})`);
      console.log(`   └─ Prediction Agent: ${m.prediction.direction.toUpperCase()} (conf: ${(m.prediction.confidence * 100).toFixed(0)}%)`);
      console.log('   ---------------------------------------------');
    }
  } else {
    console.log('❌ No markets currently hit the 40%+ confidence threshold.');
  }

  // Always show top 5 supported core/meme assets by confidence
  const supportedResults = allResults.filter(r => r.isSupported);
  supportedResults.sort((a, b) => b.fusion.confidence - a.fusion.confidence);
  console.log('\n=== 🚀 TOP 5 CORE/SUPPORTED ASSETS BY CONFIDENCE ===');
  for (const m of supportedResults.slice(0, 5)) {
    console.log(`[Supported] ${m.symbol} | fusedAction: ${m.fusion.action} | fusedConfidence: ${(m.fusion.confidence * 100).toFixed(1)}%`);
    console.log(`   └─ Technical Agent : ${m.technical.action} (conf: ${(m.technical.confidence * 100).toFixed(0)}%)`);
    console.log(`   └─ Sentiment Agent : ${m.sentiment.label.toUpperCase()} (conf: ${(m.sentiment.confidence * 100).toFixed(0)}%, articles: ${m.sentiment.articleCount})`);
    console.log(`   └─ Prediction Agent: ${m.prediction.direction.toUpperCase()} (conf: ${(m.prediction.confidence * 100).toFixed(0)}%)`);
    console.log('   ---------------------------------------------');
